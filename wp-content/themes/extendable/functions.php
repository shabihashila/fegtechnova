...
/**
 * FEG TechNova Custom Redesign - Enqueue Assets
 * Loads custom CSS, globe JS, countdown JS, and interaction JS on the front page.
 */
function feg_enqueue_redesign_assets() {
	// Load the custom CSS + core interactions site-wide so inner pages match the dark theme.
	// Globe and countdown scripts only load on the front page (their DOM only exists there).
	$is_front = is_front_page();

	// Custom CSS
	wp_enqueue_style(
		'feg-custom-style',
		get_template_directory_uri() . '/assets/css/feg-custom.css',
		array(),
		'2.2.1'
	);

	if ( $is_front ) {
		// 3D Globe canvas script
		wp_enqueue_script(
			'feg-globe',
			get_template_directory_uri() . '/assets/js/feg-globe.js',
			array(),
			'1.0.0',
			true
		);
		...
		wp_enqueue_script(
			'feg-countdown',
			get_template_directory_uri() . '/assets/js/feg-countdown.js',
			array(),
			'1.0.0',
			true
		);
...
// Scroll animations, particles, mobile nav, sticky header
	wp_enqueue_script(
		'feg-interactions',
		get_template_directory_uri() . '/assets/js/feg-interactions.js',
		array(),
		'1.3.0',
		true
	);
}
add_action( 'wp_enqueue_scripts', 'feg_enqueue_redesign_assets' );

/**
 * Tag <body> with the redesign class so the dark glassmorphism
 * palette applies to the front page and every inner page.
 */
function feg_add_redesign_body_class( $classes ) {
	$classes[] = 'feg-redesign';
	return $classes;
}
add_filter( 'body_class', 'feg_add_redesign_body_class' );
