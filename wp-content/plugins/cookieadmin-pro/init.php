...
if(!defined('SITEPAD')){
	add_filter('site_transient_update_plugins', 'cookieadmin_pro_disable_manual_update_for_plugin', 99);
	add_filter('pre_site_transient_update_plugins', 'cookieadmin_pro_disable_manual_update_for_plugin', 99);
	add_filter('transient_update_plugins', 'cookieadmin_pro_disable_manual_update_for_plugin', 99);
...
// GEO Targeting filter option table filter hooks
add_filter('option_cookieadmin_law', '\CookieAdminPro\Geo::get_active_law');
add_filter('option_cookieadmin_settings', '\CookieAdminPro\Geo::apply_geo_rule_settings');
// for default option value if the option is not saved
add_filter('default_option_cookieadmin_law', '\CookieAdminPro\Geo::get_active_law');
add_filter('default_option_cookieadmin_settings', '\CookieAdminPro\Geo::apply_geo_rule_settings');
...
// Geo DB update check
add_action('cookieadmin_daily_geo_db_update_check', '\CookieAdminPro\Cron::check_geo_db_update');
...
function cookieadmin_pro_load_plugin(){
	...
	// Cron to check if the newer country database file is available
	if(!empty($cookieadmin['license']) && !empty($cookieadmin['license']['license']) && !empty($cookieadmin['license']['active'])){
		...
		if(!empty($geo_enabled) && !wp_next_scheduled('cookieadmin_daily_geo_db_update_check')){
			wp_schedule_event(time() + DAY_IN_SECONDS, 'daily', 'cookieadmin_daily_geo_db_update_check');
		}
	}
...
function cookieadmin_pro_load_plugin_admin(){
	...
	// SHow Notice for the admin;
	if(method_exists('\CookieAdminPro\Admin\Geo', 'show_db_update_notice')){
		add_action('admin_notices', '\CookieAdminPro\Admin\Geo::show_db_update_notice', 999);
	}
}
