...
class Enduser{
	...
	static function enqueue_scripts(){
		...
		// Do not load cookies if banner is disabled via geo rule
		if(empty($view)){
			return;
		}
		...
		// GEO Targeting: pass resolved law and feature overrides to frontend
		$geo_enabled = get_option('cookieadmin_geo_enabled', false);
		if(!empty($geo_enabled)){
			$geo_overrides = \CookieAdminPro\Geo::get_feature_overrides();
			$vars['geo_feature_overrides'] = $geo_overrides;
		}
	...
	static function wp_head() {
		...
		// Do not load script if the banner is off via Geo rule
		if(empty($law)){
			return;
		}
			...
			foreach($scanned_cookies as $cookie){
				if(!empty($cookie->description)){
					if(cookieadmin_is_polylang_active()){
						...
						$cookie->description = apply_filters( 'wpml_translate_single_string', $cookie->description, 'CookieAdmin', $cookie->cookie_name);
					}
				}
			}
		}
