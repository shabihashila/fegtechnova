...
class DoNotSell{
	...
	static function submit_do_not_sell_form(){
		...
		if(empty($form_data['confirmation'])){
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_confirm_err'], 'dns_confirm_err')));
		}
		...
		if(
			...
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_empty_fields_err'], 'dns_empty_fields_err')));
		}

		if(!preg_match('/^[\p{L}\s\'-]{2,50}$/u', $form_data['first_name']) || !preg_match('/^[\p{L}\s\'-]{2,50}$/u', $form_data['last_name'])){
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_name_err'], 'dns_name_err')));
		}

		if(!is_email($form_data['email'])){
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_email_err'], 'dns_email_err')));
		}
		...
		if(!empty($form_data['zip_code']) && !preg_match('/^[a-zA-Z0-9-\s]{3,10}$/', $form_data['zip_code'])){
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_zip_err'], 'dns_zip_err')));
		}

		if(!empty($form_data['phone']) && !preg_match('/^\+?[0-9\s\-\.\(\)]{7,20}$/', $form_data['phone'])){
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_phone_err'], 'dns_phone_err')));
		}
		...
		if(!empty($email_exist)){
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_request_in_progress'], 'dns_request_in_progress')));
		}
		...
		if($result === false){
			wp_send_json_error(esc_html(cookieadmin__($cookieadmin['default']['dns_submission_err'], 'dns_submission_err')));
		}

		wp_send_json_success(esc_html(cookieadmin__($cookieadmin['default']['dns_submission_success'], 'dns_submission_success')));
	}
	...
	static function get_dns_shortcode_html(){
		...
		$output = '<div class="cookieadmin-do-not-sell-form">
			<div class="cookieadmin-do-not-sell-form-header">
				<h3>'.esc_html(cookieadmin__($cookieadmin['default']['dns_heading'], 'dns_heading')).'</h3>
			</div>
			<form method="post" id="cookieadmin_dns_form" onsubmit="cookieadmin_pro_submit_dns_form(event)">
				...
				<div class="cookieadmin-do-not-sell-form-field">
					<label for="cookieadmin_dns_first_name">'.esc_html(cookieadmin__($cookieadmin['default']['dns_fname'], 'dns_fname')).'<span  class="cookieadmin-do-not-sell-req-input">*</span></label>
					<input type="text" name="cookieadmin_dns_first_name" id="cookieadmin_dns_first_name" required>
				</div>
				
				<div class="cookieadmin-do-not-sell-form-field">
					<label for="cookieadmin_dns_last_name">'.esc_html(cookieadmin__($cookieadmin['default']['dns_lname'], 'dns_lname')).'<span  class="cookieadmin-do-not-sell-req-input">*</span></label>
					<input type="text" name="cookieadmin_dns_last_name" id="cookieadmin_dns_last_name" required>
				</div>
				
				<div class="cookieadmin-do-not-sell-form-field">
					<label for="cookieadmin_dns_email">'.esc_html(cookieadmin__($cookieadmin['default']['dns_email'], 'dns_email')).'<span  class="cookieadmin-do-not-sell-req-input">*</span></label>
				...
				if(!empty($settings['zip_enabled'])){
					$output .= '<div class="cookieadmin-do-not-sell-form-field">
						<label for="cookieadmin_dns_zip_code">'.esc_html(cookieadmin__($cookieadmin['default']['dns_zip'], 'dns_zip')).'<span>'.(!empty($settings['zip_required']) ? '<span class="cookieadmin-do-not-sell-req-input">*</span>' : '').'</span></label>
				...
				if(!empty($settings['phone_enabled'])){
					$output .= '<div class="cookieadmin-do-not-sell-form-field">
						<label for="cookieadmin_dns_phone">'.esc_html(cookieadmin__($cookieadmin['default']['dns_phone'], 'dns_phone')).'<span>'.(!empty($settings['phone_required']) ? '<span class="cookieadmin-do-not-sell-req-input">*</span>' : '').'</span></label>
				...
				$output .= '<div class="cookieadmin-do-not-sell-form-field">
					...
					<span class="cookieadmin-do-not-sell-req-input">*</span>'.esc_html(cookieadmin__($cookieadmin['default']['dns_confirm_msg'], 'dns_confirm_msg')).'</span>
				</div>
				<div class="cookieadmin-do-not-sell-form-field cookieadmin-do-not-sell-save-btn" style="flex-direction:row;gap:10px; align-items:center;">
					<input type="submit" name="cookieadmin_dns_submit" class="button button-primary btn btn-primary wp-element-button cookieadmin_dns_submit" id="cookieadmin_dns_submit" value="'.esc_html(cookieadmin__($cookieadmin['default']['dns_submit'], 'dns_submit')).'">
